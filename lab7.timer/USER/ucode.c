...
char *cmd[]={"getpid", "ps", "chname", "kmode", "switch", "wait", "exit", "fork", "exec", "pipe", "pfd", "rpipe", "wpipe", "cpipe", "sleep", 0};
...
{
 printf("***********************  Menu  **********************\n");
 printf("*  ps chname kmode switch wait exit fork exec sleep *\n");
 printf("*****************************************************\n");
 printf("\n");
}
...
int do_sleep()
{
  char s[16];
  int t;
  printf("input sleep time (seconds) : ");
  gets(s);
  t = atoi(s);
  return syscall(14, t);
}
